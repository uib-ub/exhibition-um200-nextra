...
import { getUniqueSlug } from "./slug.js";
...
export const buildFacets = async (labels, metadata, manifestData, baseUrl) => {
  ...
  try {
    ...
    if (!existsSync(`${api}/facet`)) mkdirSync(`${api}/facet`);

    canopyFacets?.forEach((label) => {
      const labelDirectory = `${api}/facet/${label.slug}`;
      const labelCollection = buildFacetLabelCollection(label, baseUrl);

      if (!existsSync(labelDirectory)) mkdirSync(labelDirectory);
      fs.writeFile(
        `${labelDirectory}.json`,
      ...
      label.values.forEach((value) => {
        ...
        const valueCollection = buildFacetValueCollection(
          value,
...
export const buildFacetLabelCollection = (label, baseUrl) => {
...
export const buildFacetValueCollection = (value, label, baseUrl, manifests) => {
